...
ClassCollage 是一个课堂照片一键拼接美化工具。把 1 到 6 张课堂照片放入 `input/`，运行命令后会生成一张适合发给家长的课堂记录图片。
...
如果 `input/` 或 `output/` 不存在，程序会自动创建。图片少于 1 张时会提示：
...
- 1 张图时会按原图尺寸生成结果图，只在上下左右加少量留白；顶部仅保留文案“孩子们开始上课啦”，不使用英文祝福、底部文案、画布边框和单张照片 PNG 相框，并会随机放置 3~4 个贴纸在照片边缘。
